...
class GuildMemberAddSendSurvey extends Listener {
  ...
  async run(member) {
    ...
    try {
      ...
      await user.send({ embeds: [this.buildSurveyMessage(joinedUser)] });
    } catch (error) {
      logger.error(`The following error occurred while attempting to send the survey to a new member ${error.message}`);
    }
  }
